# problemSolving
## tip
    전역변수 사용시 반드시 solution 안에서 한번 초기화 해주기. 전역변수 선언할 때 초기화 해줬어도 다시 한번 해줘야한다.
...
## 정렬
    해당 문제)
    백준 -> 10989 수 정렬하기 3
    그냥 벡터쓰면 되는 문제인줄 알았는데 메모리가 터지게 해놨다. 이건 입력받은 숫자를 인덱스로하는 배열의 수를 카운트해서 0이 아니면 출력하는 형태로 하면 메모리 크기를 천만 -> 만으로 줄일 수 있다.
...
## BFS
    ...
    programers -> level4 -> 게임 맵 최단거리
    토마토 문제의 교훈을 되세기자
    dfs, bfs든 풀 수는 있는데 bfs가 가장먼저 도달하는게 최단경로임이 보장됨을 이용해야 시간초과를 피할 수 있다.
    그리고 이렇게 bfs를 queue를 사용해서 풀 경우에는 반드시! 방문 노드의 마킹을 push 를 진행하는 부분에 넣어주자. 즉, 사전에 마킹함으로써 불 필요한 방문을 사전에 막는 것이다.
    코드가 깔끔하게 짜지진 않지만 속도가 드라마틱하게 상승! 한다.
...
## DFS
    ...
    (마킹 형식이 아니라 매 지점마다 check 했던 풀이의 경우)퀸을 둘 수 없는 자리를 마킹할 때 위에서 아래로 탐색함으로 아래지점은 당연히 비어있기 때문에 현재의 위쪽 세로, 오른쪽 대각선, 왼쪽 대각선 이렇게 세 방향으로만 탐색을 진행해 연산 수를 줄일 수 있다. 이전엔 가로, 세로 대각선 위 아래 모두 탐색했기 때문에 2배 더 비효율 적이었다. 하지만 문제의  핵심은 check가 아니었다.
...
## 크루스칼
    ...
    find로 연결된 노드의 끝을? 탐색하는 개념을 활용해서 문제 해결.
    현재 노드를 체크하고 이 노드가 중복해서 체크될 때 다음 노드에 대한 위치를 미리 준비해놓는다.
...
## 다익스트라
    ...
    input을 줄 때 양 방향으로 움직이는지, 같은 경로에 다른 값으로 중복되어 값이 들어오는지를 꼭 확인해보자.
    ...
    해당문제)
    백준 -> 다익스트라 -> K번째 최단경로 찾기

    기존의 다익스트라에서 관리하는 distance에서 k번째 최적화된 경로까지 담아야 하는 것이 핵심이다.
    이 문제에서 이를 pq로 다뤘는데 값들을 정렬시켜서 k번째 이상 값을 쳐내기 위해서 그렇다. 만약 k번째 값보다 작은 값이 들어오게 된다면 기존 top에 있는 값을 제거하고 새로운 값을 넣으면 된다.
...
## n개의 리스트 중에서 가장 큰 값을 뽑는 방법
    해당문제)
    programers -> ?? -> 5번
    기본적으로는 리스트를 순회해서 Max를 갱신하는 것.
    만약 시간초과가 터지는 경우라면 pq를 이용해 최고 값을 push마다 갱신하도록 한다. dp가 섞여서 제약사항이 생긴다면
    pair로 second 인자로 조건을 관리해서 최고 값이지만 조건에 부합하지 않는다면 자르는 식으로 한다.
...
## 다시 볼 문제
    ...
    programers -> level2 -> 위장
    -> 서로 다른 옷 종류가 각각 3개, 2개, 2개 있다고 했을 때 입을 수 있는 경우의 수는 옷을 안 입는 경우 +1을 각각한 후 곱한 4 * 3 * 3 의 값에서 하나도 안입은 경우를 뺀 것으로 수식으로 구할 수 있다.
    ...
    programers -> level2 -> 방금그곡(카카오)
    -> 시간 계산할 때 분만 고려해서 풀었다가 틀림. 시간이 2시간 이상 차이 날 경우를 고려해서 풀면 된다.
    programers -> level1 -> 시저암호
    -> 이유는 모르겠지만 char형으로 풀면 터지는 문제가 있는듯. int 로 캐스팅해서 풀면 해결됨.
    programers -> level4 -> 호텔 방 배정(카카오 2019 4번)
    -> 크루스칼 처럼 재귀적인 find를 통해서 다음 노드를 기록한다. 선형적으로 빈 노드를 계산하면 시간안에 동작하지 않는다.
    programers -> level2 -> 위장
    -> 경우의 수 세는법... 옷을 안 입는 경우 까지 포함해서 곱 연산 후 모두 안 입는 하나의 경우를 뺀다.
    programers -> level2 -> 추석트래픽(카카오)
    -> 범위를 구하는 조건에서 3가지 조건을 충족시키는거보다 한가지 조건을 만족하지 않는 경우의 여집합으로 체크하는게 더 편함.2016-09-15인 로그를 무시해서 줄임. 소수점은 float로 그냥 처리하자. 마지막으로 범위 순회에서는 로그 데이터이기 때문에 굳이 모든 범위를 순회하지 말고 검사하려는 구간의 시작지점을 로그데이터의 시작, 끝 지점으로 하여 86400000번 순회하는 바보같은 짓을 하지 말자!
    programers -> level4 -> 게임 맵 최단거리
    -> 토마토의 교훈을 다시 되세기자!. 일단 이러한 최단경로 문제는 dfs가 아닌 bfs를 쓰고 가장먼저 목적지에 도달한 것이 곧 정답임으로 q를 break해서 끝낸다.
    baekjoon -> 단계별로 풀어보기 -> 부르트포스 -> 덩치
    -> 너무 어렵게 생각하지 말고 무식하게 풀자. 이러한 조건 검사는 2중 for문으로 모두 탐색하고 값을 갱신해간다기 보다는 한 loop마다 새로 값을 구한다는 생각으로 문제를 해결하자.
    programers -> level2 -> 가장 큰 수
    -> 스트링의 비교에서 a+b, b+a로 비교해서 손쉽게 크기 비교할 수 있다. 최종결과로 숫자 형태를 받아야 함으로 맨 앞에 0이 오는 경우는 오직 0이 나열된 경우 뿐임으로 "0"으로 갱신해줘야 한다.
